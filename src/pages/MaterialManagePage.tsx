...
const materialCategories = ['贵金属', '宝石', '有机材质', '工艺材质']
...
export default function MaterialManagePage() {
  ...
  const [form, setForm] = useState({ name: '', nameEn: '', category: '贵金属', description: '' })

  const allItems = materials || []
  const filtered = filterCat === '全部' ? allItems : allItems.filter(m => m.category === filterCat)

  const resetForm = () => { setForm({ name: '', nameEn: '', category: '贵金属', description: '' }); setEditingId(null); setShowAdd(false) }
  ...
  return (
    <div className="fade-in">
          ...
          {['全部', ...materialCategories].map(cat => (
            <button
              ...
              className={`px-3 sm:px-4 py-2 rounded-xl text-xs sm:text-sm font-medium cursor-pointer transition-colors ${
                filterCat === cat ? 'bg-primary-500 text-white' : 'bg-white/60 text-ink-500 hover:bg-ivory-100 active:bg-ivory-200 border border-ivory-200'
              }`}
            >
              {cat}
              <span className="ml-1 sm:ml-1.5 text-xs opacity-75">
                {cat === '全部' ? allItems.length : allItems.filter(m => m.category === cat).length}
              </span>
            </button>
          ))}
        </div>
      ...
      {showAdd && (
        <div className="px-4 sm:px-6 md:px-8 mb-6">
          <GlassCard>
            <h3 className="font-heading text-lg font-semibold text-ink-800 mb-4">{editingId !== null ? '编辑材质' : '添加材质'}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="如：黄金" className="input-field w-full rounded-xl px-4 py-3 text-sm" />
              </div>
              <div>
              ...
              <div>
                ...
                <select value={form.category} onChange={e => setForm({ ...form, category: e.target.value })} className={`input-field w-full rounded-xl px-4 py-3 text-sm cursor-pointer appearance-none ${selectArrowBg}`}>
              ...
              <div>
                ...
                <input type="text" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="材质特征简述" className="input-field w-full rounded-xl px-4 py-3 text-sm" />
              </div>
            </div>
      ...
      <div className="md:hidden px-4 sm:px-6 pb-10 space-y-3">
        {filtered.map(mat => (
          <GlassCard key={mat.id} className="!p-4">
            ...
            <div className="flex items-center gap-2 mb-2">
              <span className={`tag ${categoryColors[mat.category] || 'bg-ivory-200 text-ink-600'}`}>{mat.category}</span>
            </div>
            ...
            <div className="flex items-center gap-4 pt-2 border-t border-ivory-200/60">
              <button onClick={() => handleEdit(mat)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm text-ink-500 hover:text-primary-500 hover:bg-primary-50/50 cursor-pointer transition-colors">
              ...
              <button onClick={() => handleDelete(mat.id)} className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-sm text-ink-500 hover:text-accent-500 hover:bg-accent-50/50 cursor-pointer transition-colors">
        ...
        {filtered.length === 0 && <div className="text-center py-12 text-ink-400"><p>暂无{filterCat === '全部' ? '' : `"${filterCat}"类`}材质数据</p></div>}
      </div>
              ...
              <tbody>
                {filtered.map(mat => (
                  <tr key={mat.id} className="table-row border-b border-ivory-100 last:border-0">
                    ...
                    <td className="px-6 py-4"><span className={`tag ${categoryColors[mat.category] || 'bg-ivory-200 text-ink-600'}`}>{mat.category}</span></td>
                    <td className="px-6 py-4 text-ink-400 hidden lg:table-cell max-w-xs truncate">{mat.description}</td>
                    <td className="px-6 py-4 text-center"><span className="tag bg-primary-50 text-primary-600">{mat.count}</span></td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex items-center justify-end gap-1">
                        <button onClick={() => handleEdit(mat)} className="text-ink-400 hover:text-primary-500 cursor-pointer p-2 rounded-lg hover:bg-primary-50/50" title="编辑">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" /></svg>
                        </button>
                        <button onClick={() => handleDelete(mat.id)} className="text-ink-400 hover:text-accent-500 cursor-pointer p-2 rounded-lg hover:bg-accent-50/50" title="删除">
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" /></svg>
                        </button>
                      </div>
                    </td>
