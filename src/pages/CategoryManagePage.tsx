...
export default function CategoryManagePage() {
  ...
  const resetForm = () => { setForm({ name: '', nameEn: '', description: '' }); setEditingId(null); setShowAdd(false) }
    ...
    <div className="fade-in">
      <div className="px-4 sm:px-6 md:px-8 pt-6 sm:pt-8 pb-4">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3">
          <div>
            <h2 className="font-heading text-2xl sm:text-3xl font-bold text-ink-800">品类管理</h2>
      ...
      <div className="px-4 sm:px-6 md:px-8 grid grid-cols-3 gap-3 sm:gap-4 mb-6">
        <div className="stat-card rounded-2xl p-3 sm:p-4 text-center">
          <p className="text-xs sm:text-sm text-ink-400">品类总数</p>
          <p className="text-xl sm:text-2xl font-bold text-ink-800 font-heading mt-0.5 sm:mt-1">{items.length}</p>
        </div>
        <div className="stat-card rounded-2xl p-3 sm:p-4 text-center">
        ...
        <div className="stat-card rounded-2xl p-3 sm:p-4 text-center">
          <p className="text-xs sm:text-sm text-ink-400">平均每类</p>
          <p className="text-xl sm:text-2xl font-bold text-ink-800 font-heading mt-0.5 sm:mt-1">{items.length ? Math.round(totalCount / items.length) : 0}</p>
        </div>
      </div>
      ...
      {showAdd && (
        <div className="px-4 sm:px-6 md:px-8 mb-6">
          <GlassCard>
            <h3 className="font-heading text-lg font-semibold text-ink-800 mb-4">{editingId !== null ? '编辑品类' : '添加品类'}</h3>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-4">
              <div>
                ...
                <input type="text" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="如：戒指" className="input-field w-full rounded-xl px-4 py-3 text-sm" />
              </div>
              <div>
                <label className="block text-sm font-medium text-ink-600 mb-1.5">英文名称</label>
                <input type="text" value={form.nameEn} onChange={e => setForm({ ...form, nameEn: e.target.value })} placeholder="如：Ring" className="input-field w-full rounded-xl px-4 py-3 text-sm" />
              </div>
              <div className="sm:col-span-2 md:col-span-1">
                <label className="block text-sm font-medium text-ink-600 mb-1.5">描述</label>
                <input type="text" value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} placeholder="品类包含的细分类型" className="input-field w-full rounded-xl px-4 py-3 text-sm" />
              </div>
            </div>
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-10">
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
          {items.map(cat => (
            <GlassCard key={cat.id} className="!p-4 sm:!p-5">
              <div className="flex items-start justify-between mb-2 sm:mb-3">
                <div className="min-w-0 mr-2">
                  <h4 className="font-heading text-base sm:text-lg font-semibold text-ink-800">{cat.name}</h4>
                  <p className="text-xs text-ink-400 mt-0.5">{cat.nameEn}</p>
                </div>
                <span className="tag bg-primary-50 text-primary-600 text-sm sm:text-base font-semibold flex-shrink-0">{cat.count}</span>
              </div>
              <p className="text-sm text-ink-400 mb-3 sm:mb-4 line-clamp-2">{cat.description}</p>
              <div className="flex items-center gap-4 pt-2 border-t border-ivory-200/60">
                <button onClick={() => handleEdit(cat)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs sm:text-sm text-ink-500 hover:text-primary-500 hover:bg-primary-50/50 active:bg-primary-50 cursor-pointer transition-colors">
                ...
                <div className="w-px h-4 bg-ivory-200" />
                <button onClick={() => handleDelete(cat.id)} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded-lg text-xs sm:text-sm text-ink-500 hover:text-accent-500 hover:bg-accent-50/50 active:bg-accent-50 cursor-pointer transition-colors">
