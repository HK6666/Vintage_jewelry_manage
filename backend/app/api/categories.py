...
def list_categories():
    logger.info(f"GET /categories - args: {dict(request.args)}")
    keyword = request.args.get('keyword', '')
    query = Category.query
    if keyword:
        query = query.filter(db.or_(
            Category.name.ilike(f'%{keyword}%'),
            Category.name_en.ilike(f'%{keyword}%'),
        ))
    items = query.order_by(Category.id.asc()).all()
...
def get_category(id):
    ...
    item = db.session.get(Category, id)
    if not item:
        return error('品类不存在', 404)
...
def create_category():
    ...
    logger.info(f"POST /categories - name: {data.get('name')}")

    if not data.get('name'):
        return error('名称不能为空', 400)
    if Category.query.filter_by(name=data['name']).first():
        return error('该品类已存在', 409)
    ...
    try:
        db.session.add(item)
        db.session.commit()
        logger.info(f"Created category id={item.id}")
...
def update_category(id):
    data = request.get_json(silent=True) or {}
    logger.info(f"PUT /categories/{id}")
    item = db.session.get(Category, id)
    if not item:
        return error('品类不存在', 404)
    ...
    try:
        db.session.commit()
        logger.info(f"Updated category id={id}")
...
def delete_category(id):
    ...
    if item.collections.count() > 0:
        return error('该品类下存在关联藏品，请先解除关联', 409)
...
def batch_delete_categories():
    data = request.get_json(silent=True) or {}
    ids = data.get('ids', [])
    logger.info(f"DELETE /categories/batch - ids: {ids}")
    if not ids:
        return error('请提供要删除的ID列表', 400)

    from ..models.collection import Collection
    conflict = Category.query.filter(Category.id.in_(ids)).join(Collection).first()
    if conflict:
        return error('部分品类存在关联藏品，请先解除关联', 409)

    try:
        Category.query.filter(Category.id.in_(ids)).delete(synchronize_session=False)
        db.session.commit()
        logger.info(f"Batch deleted categories: {ids}")
        return success(message='批量删除成功')
    except Exception as e:
        db.session.rollback()
        logger.error(f"Failed to batch delete categories: {e}", exc_info=True)
