...
import { defaultBrands, type BrandItem } from '../data/collections'
...
export default function BrandManagePage() {
  ...
  const resetForm = () => {
    setForm({ name: '', nameEn: '', country: '', description: '' })
    setEditingId(null)
    setShowAdd(false)
  }
  ...
  const handleSave = () => {
    ...
    if (editingId !== null) {
      setBrands(brands.map(b => b.id === editingId ? { ...b, ...form } : b))
    } else {
      const newId = Math.max(...brands.map(b => b.id), 0) + 1
      setBrands([...brands, { id: newId, ...form, count: 0 }])
    }
    resetForm()
  }
      ...
      <div className="md:hidden px-4 sm:px-6 pb-10 space-y-3">
        {brands.map(brand => (
          <GlassCard key={brand.id} className="!p-4">
            ...
            <div className="flex items-center gap-2 mb-2">
              <span className="tag bg-blue-50 text-blue-700">{brand.country}</span>
            </div>
        ...
        {brands.length === 0 && (
          <div className="text-center py-12 text-ink-400">
            <p>暂无品牌数据</p>
            <p className="text-sm mt-1">点击"添加品牌"开始录入</p>
          </div>
        )}
      </div>
                ...
                {brands.map(brand => (
                  <tr key={brand.id} className="table-row border-b border-ivory-100 last:border-0">
                    ...
                    <td className="px-6 py-4 text-center">
                      <span className="tag bg-primary-50 text-primary-600">{brand.count}</span>
                    </td>
