...
import { defaultColors, type ColorItem } from '../data/collections'
...
export default function ColorManagePage() {
  ...
  const resetForm = () => {
    setForm({ name: '', nameEn: '', hex: '#D4A853', description: '' })
    setEditingId(null)
    setShowAdd(false)
  }
  ...
  const handleSave = () => {
    ...
    if (editingId !== null) {
      setColors(colors.map(c => c.id === editingId ? { ...c, ...form } : c))
    } else {
      const newId = Math.max(...colors.map(c => c.id), 0) + 1
      setColors([...colors, { id: newId, ...form, count: 0 }])
    }
    resetForm()
  }
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    type="color"
                    value={form.hex.startsWith('#') ? form.hex : '#D4A853'}
                    onChange={e => setForm({ ...form, hex: e.target.value })}
                    className="w-10 h-10 rounded-lg border border-ivory-200 cursor-pointer flex-shrink-0"
                  />
                  <input
                    type="text"
                    value={form.hex}
      ...
      <div className="px-4 sm:px-6 md:px-8 pb-10">
          ...
          {colors.map(color => (
            <GlassCard key={color.id} className="!p-0 overflow-hidden">
              ...
              <div
                ...
                style={{
                  background: isGradient(color.hex) ? color.hex : color.hex,
                  ...(color.hex === '#F5F5F5' ? { border: '1px solid #e5e5e5', borderBottom: 'none' } : {}),
                }}
              />
              <div className="p-4">
                ...
                <div className="flex items-center gap-2 mb-2">
                  {!isGradient(color.hex) && (
                    <span className="text-xs text-ink-400 font-mono">{color.hex}</span>
                  )}
                </div>
        ...
        {colors.length === 0 && (
          <div className="text-center py-12 text-ink-400">
            <p>暂无色系数据</p>
            <p className="text-sm mt-1">点击"添加色系"开始录入</p>
          </div>
        )}
      </div>
    </div>
